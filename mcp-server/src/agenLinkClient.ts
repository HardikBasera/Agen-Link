 ...
 */
function connectWithRetry(port: number, budgetMs = CONNECT_BUDGET_MS): Promise<net.Socket> {
    ...
    const tryOnce = () => {
      ...
      const onError = (err: Error) => {
        ...
        if (elapsed >= budgetMs) {
          reject(
            new Error(
              `Cannot reach the Agen-Link on 127.0.0.1:${port} (${err.message}). Retried for ` +
...
export function agenLinkRequest(
  ...
  return connectWithRetry(port).then(
    (socket) =>
      new Promise<unknown>((resolve, reject) => {
        ...
        socket.on("error", (err: Error) => {
          finish(() =>
            reject(
              new Error(
                `Lost the Agen-Link connection on 127.0.0.1:${port} (${err.message}). ` +
