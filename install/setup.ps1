<#
  Agen-Link — one-time setup.
  Builds the MCP server and installs the GitHub CLI (gh). Run from anywhere:
...
Write-Host "[1/4] Building the MCP server..." -ForegroundColor Cyan
if (-not (Get-Command node -ErrorAction SilentlyContinue)) { throw "Node.js not found. Install Node 18+ first (https://nodejs.org)." }
...
Write-Host "`n[2/4] Installing the pty-host (Terminal)..." -ForegroundColor Cyan
Push-Location $pty
try {
    npm install
}
...
Write-Host "`n[3/4] GitHub CLI (gh)..." -ForegroundColor Cyan
