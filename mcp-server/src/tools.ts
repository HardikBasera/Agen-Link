...
export const tools: ToolDef[] = [
  {
    name: "agen_ping",
    description:
      "Health probe. Answers from the bridge's socket thread WITHOUT touching Unity's main thread, so it " +
  ...
  },
  {
    ...
    description:
      "Get the active scene's GameObject hierarchy: instanceID, name, hierarchy path, active state, attached " +
  ...
  },
  {
    ...
    description:
      "Get the objects currently selected in the Unity Editor (names, types, and asset paths if any). " +
  ...
  // throwaway editor script, and must query the scene/console with the read tools instead of asking the user.
  {
    ...
    description:
      "Create a GameObject in the open scene — use THIS, never a C# editor script, to add objects. Make it " +
    ...
    run: (port, a) =>
      agenLinkRequest(port, "create_gameobject", {
        name: a.name ?? "",
  ...
  },
  {
    ...
    description:
      "Modify an existing scene GameObject: rename, reparent (parent:'' moves to scene root), set active " +
  ...
  },
  {
    ...
    description:
      "Delete one or more scene GameObjects. Each target is an instanceID, hierarchy path, or name. " +
  ...
  },
  {
    ...
    description:
      "Find GameObjects in the loaded scenes (including inactive) by name substring, exact hierarchy path, " +
    ...
    run: (port, a) =>
      agenLinkRequest(port, "find_gameobjects", {
        gname: a.name ?? "",
        gpath: a.path ?? "",
  ...
  },
  {
    ...
    description:
      "Get one GameObject's full data: every component with its serialized property names and current values — " +
    ...
    run: (port, a) =>
      agenLinkRequest(port, "get_gameobject", {
        target: a.target,
  ...
  },
  {
    ...
    description:
      "Add or remove a component on a GameObject. componentType accepts a full name ('UnityEngine.Rigidbody') " +
    ...
    run: (port, a) =>
      agenLinkRequest(port, "manage_component", {
        action: a.action,
        target: a.target,
  ...
  },
  {
    ...
    description:
      "Set serialized fields on a component, exactly like editing the Inspector (Undo-able; scene marked dirty, " +
  ...
  },
  {
    ...
    description:
      "Save, open, or create Unity scenes (action: save|open|create). save writes the active scene to disk — " +
    ...
    run: (port, a) =>
      agenLinkRequest(port, "manage_scene", {
        action: a.action,
        path: a.path ?? "",
  ...
  },
  {
    ...
    description:
      "Asset operations that keep GUIDs and .meta files consistent — NEVER move/copy/delete Unity assets with " +
  ...
  },
  {
    ...
    description:
      "Execute a Unity Editor menu item by its exact path, e.g. 'GameObject/UI/Button' or 'Assets/Refresh' — the " +
  ...
  },
  {
    ...
    description:
      "Select objects in the Editor and ping-highlight the first so the user can SEE what you mean — do this " +
  ...
  },
  {
    ...
    description:
      "Capture the Game view (main camera; live gameplay if playing) or the Scene view to a PNG under " +
    ...
    run: (port, a) =>
      agenLinkRequest(port, "capture_screenshot", { view: a.view ?? "game", width: a.width ?? 1280, height: a.height ?? 720 }),
  },
  {
    name: "agen_run_tests",
    description:
      "Run Unity tests through the Test Runner. action:'start' with mode EditMode|PlayMode and an optional " +
    ...
    run: (port, a) =>
      agenLinkRequest(port, "run_tests", { action: a.action, mode: a.mode ?? "EditMode", testFilter: a.testFilter ?? "" }),
  },
  {
    name: "agen_execute_code",
    description:
      "Compile and run a short C# snippet inside the Editor immediately (no script files, no domain reload) — " +
