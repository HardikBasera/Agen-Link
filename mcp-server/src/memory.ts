...
// Shared, model-agnostic project memory. These tools are pure filesystem ops — deliberately NOT routed
// through the Unity bridge — so they work even when the Editor is "parked" (unfocused) and stalls bridge
// calls. Both the Claude and Antigravity CLIs point at this same MCP server, so both read/write one store.
...
const author = (): string => process.env.AGEN_LINK_CLI ?? "unknown";
const memoryDir = (): string => join(projectRoot(), "AgenLink~", "memory");
const storePath = (): string => join(memoryDir(), "memory.jsonl");
...
function readEntries(): MemoryEntry[] {
  ...
  for (const line of readFileSync(p, "utf8").split("\n")) {
    ...
    if (!s) continue;
    try {
      out.push(JSON.parse(s) as MemoryEntry);
    } catch {
      /* skip malformed line */
    }
  }
...
export const memoryTools: ToolDef[] = [
  {
    ...
    description:
      "Append a durable note to the SHARED project memory (read by both the Claude and Antigravity CLIs working " +
  ...
  },
  {
    ...
    description:
      "Search the SHARED project memory BEFORE scanning the project. Returns notes (from either CLI) whose " +
